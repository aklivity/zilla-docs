...
- Additional metadata is required during topic creation or alteration to store virtual partition count.

### Virtual Consumer Groups
- Virtual consumer groups are mapped to system consumer groups (`one-to-one`).
- System offsets are managed while ensuring virtual consumer groups skip excluded system message offsets to report lag accurately.
