...
- an VPC security group for Zilla Plus proxies
- an IAM security role for Zilla Plus proxies
...
We need to import the trusted client certificate and corresponding private key into the local key store used by the Kafka client when connecting to the Zilla Plus proxy.
...
The Zilla Plus proxy relies on TLS so we need to create a file called `client.properties` that tells the Kafka client to use SSL as the security protocol and to specify the key store containing authorized client certificates.
...
1. The Zilla Plus proxy verified the client identity of the Kafka client
1. The Zilla Plus proxy selected a matching client certificate to propagate client identity
