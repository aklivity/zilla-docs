...
Zilla supports bindings between protocols. This characteristic let you map stream from gRPC server into SSE and much more. Currently, we support these protocols:
...
Before we get through each protocol, let's discuss about bindings first.
...
- **Proxy Bindings:** A `proxy` handles the translate or encode behaviors between components.
