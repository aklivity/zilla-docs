...
A Zilla MQTT broker is defined using three specific Kafka topics. The [messages](http://34.48.98.66/kafka/ui/clusters/zilla-quickstart-kafka/all-topics/mqtt-messages) Kafka topic will have all of the MQTT messages sent to the broker, where the MQTT topic is the Kafka message `key` and the MQTT payload is the Kafka message value. Marking messages with the `retain` flag set to true will produce a message on the [retained](http://34.48.98.66/kafka/ui/clusters/zilla-quickstart-kafka/all-topics/mqtt-retained/messages) Kafka topic. The [sessions](http://34.48.98.66/kafka/ui/clusters/zilla-quickstart-kafka/all-topics/mqtt-sessions/messages) Kafka topic is used to manage MQTT client connections.
